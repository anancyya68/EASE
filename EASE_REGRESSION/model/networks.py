...
# combine model
def combined_model(embed_dim, output_shape, num_heads):
    """
    :param embed_dim:
    :param output_shape:
    :param num_heads:
    ...
    # 1.attention
    att = PreAttention(embed_dim, num_heads)
    #att.build(input_shape=(None, None, embed_dim))
    ...
    neural_net = NeuralNetwork(output_shape)
    nn_output = neural_net.mlp_method(flattened_output)
    model = Model(inputs=inputs, outputs=nn_output)
    return model
